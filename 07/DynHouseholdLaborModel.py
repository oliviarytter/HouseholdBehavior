...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        par.rho_n_1 = 0.0  # extra disutility per child, men
        par.rho_n_2 = 0.05 # extra disutility per child, women
        ...
        # children
        par.p_n = 0.5  # probability of child arriving if n=0
        par.Nn = 2     # n in {0, 1}

        # child-related transfers (set to 0 as baseline)
        par.uncon_uni   = 0.0  # C1: unconditional universal (>0)
        par.means_level = 0.0  # C2: means-tested level (>0)
        par.means_slope = 0.0  # C2: means-tested slope
        par.cond        = 0.0  # C3: net childcare cost, all working couples (<0)
        par.cond_high   = 0.0  # C4: extra cost, high-income working couples (<0)
    ...
    def allocate(self):
        ...
        sim.n  = np.zeros(shape, dtype=int)   # new
    ...
    def solve(self):
        ...
        for t in reversed(range(par.T)):
            for i_n, n in enumerate(range(par.Nn)):
                ...
                for i_k1,capital1 in enumerate(par.k_grid):
                    ...
                    # ii. find optimal hours of both members at this level of human capital in period t.
                        if t==(par.T-1): # last period
                         obj = lambda x: - self.util(x[0],x[1],capital1,capital2,n)

                        else:
                            obj = lambda x: -self.value_of_choice(x[0], x[1], capital1, capital2, n, sol.V[t+1])  
    ...
    def value_of_choice(self,hours1,hours2,capital1,capital2,n,V_next):

        # a. unpack
        par = self.par

        # b. current utility
        util = self.util(hours1,hours2,capital1,capital2,n)
        
        # c. continuation value
        k1_next = (1.0-par.delta)*capital1 + hours1
        k2_next = (1.0-par.delta)*capital2 + hours2
        
        p = par.p_n if n == 0 else 0.0
        n_plus = min(n+1, par.Nn-1)

        EV = (p     * interp_2d(par.k_grid, par.k_grid, V_next[n_plus], k1_next, k2_next)
        + (1-p) * interp_2d(par.k_grid, par.k_grid, V_next[n],      k1_next, k2_next))

        # d. return value of choice
        return util + par.beta * EV


    # relevant functions
    def consumption(self,hours1,hours2,capital1,capital2,n):
        par = self.par
        ...
        C = self.child_tran(hours1, hours2, income_hh, n)
        tax_hh = self.tax_func(income_hh + C)

        return income_hh - tax_hh + C
    ...
    def util(self,hours1,hours2,capital1,capital2,n):
        par = self.par

        cons = self.consumption(hours1,hours2,capital1,capital2,n)

        rho1 = par.rho_1 + par.rho_n_1 * n
        rho2 = par.rho_2 + par.rho_n_2 * n
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            for t in range(par.simT):
                ...
                idx_sol = t
                sim.h1[i,t] = interp_2d(par.k_grid, par.k_grid, sol.h1[t,n], sim.k1[i,t], sim.k2[i,t])
                sim.h2[i,t] = interp_2d(par.k_grid, par.k_grid, sol.h2[t,n], sim.k1[i,t], sim.k2[i,t])
                ...
                if t < par.simT-1:
                    ...
                    p = par.p_n if n == 0 else 0.0
                    sim.n[i,t+1] = n+1 if np.random.rand() < p else n
