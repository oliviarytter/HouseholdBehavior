...
class PortfolioModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        # added to answer questions
        par.tau = 0.15 # tax rate on capital gains
        par.Nr = 2 # number of states of the economy
        par.gamma = 0.8
        par.prob_recession_0 = 0.1
        par.prob_recession_1 = 0.8
    ...
    def allocate(self):
        ...
        if par.Nr>1:
            par.recession_grid = np.arange(par.Nr)
        else:
            par.recession_grid = np.array([0])
        ...
        # d. solution arrays
        shape = (par.T,par.Nr,par.Ns,par.NRb)
        ...
        sim.R = np.nan + np.zeros(shape) # individual returns
        sim.r = np.zeros(shape,dtype=np.int_) # recession state
        ...
        sim.draws_r = np.random.uniform(size=par.T)

        # g. initialization
        sim.init_s = 0.5 + np.random.uniform(size=par.simN)
        sim.init_Rb = par.mu0 + np.zeros(par.simN)
        sim.init_r = np.zeros(par.simN,dtype=np.int_) # recession state
    ...
    def solve(self):
        ...
        for t in reversed(range(par.T-1)):
            ...
            for i_r,recession in enumerate(par.recession_grid):
                for i_s,savings in enumerate(par.s_grid):
                    for i_Rb,Rb in enumerate(par.Rb_grid):
                        idx = (t,i_r,i_s,i_Rb) # index for current period, savings and risky asset return
                        idx_last = (t+1,i_r,i_s,i_Rb) # index for next period, savings and risky asset return
                        ...
                        obj = lambda choice: - self.value_of_choice(choice[0],choice[1],savings,Rb,recession,t)  
                        ...
                        obj0 = lambda cons: - self.value_of_choice(cons[0],0.0,savings,Rb,recession,t)
                        obj1 = lambda cons: - self.value_of_choice(cons[0],1.0,savings,Rb,recession,t)
    ...
    def value_of_choice(self,cons,x_share,savings,Rb,recession,t):
        ...
        # loop over risky asset return shocks and recession state
        EV_next = 0.0
        for i_r_next,recession_next in enumerate(par.recession_grid):
            for i_eps,eps in enumerate(par.eps_grid):
                prob_eps = par.eps_weight[i_eps]
                if par.Nr>1:
                    prob_recession = par.prob_recession_0 if recession==0 else par.prob_recession_1
                    if recession_next==0:
                        prob_recession = 1.0 - prob_recession
                else:
                    prob_recession = 1.0

                # risky asset return next period
                Rb_next = self.Rb_next_func(recession,Rb,eps)
                ...
                EV_next += prob_recession * prob_eps * V_next_interp
    ...
    def a_next_func(self,resources,x_share):
        ...
        Ra_net = par.Ra - par.tau*np.fmax(par.Ra-1,0.0)
    ...
    def b_next_func(self,resources,x_share,Rb_next):
        ...
        Rb_next_net = Rb_next - par.tau*np.fmax(Rb_next-1,0.0)

        b_share = x_share * resources
        b_next = Rb_next_net * b_share
        return b_next
    
    def Rb_next_func(self,recession,Rb,eps):
        # risky asset return next period
        par = self.par
        if par.Nr>1:
            gamma = par.gamma if recession==0 else 0.0
        else:
            gamma = 1.0
        Rb_next = np.fmax(gamma*Rb + eps , 0.0)
        return Rb_next
    ...
    def simulate(self):
        ...
        for i in range(par.simN):
            ...
            sim.r[i,0] = sim.init_r[i]

            for t in range(par.simT):
                if t<par.T: # check that simulation does not go further than solution                 
                    
                    r = sim.r[i,t] # recession state

                    # iii. interpolate optimal consumption and portfolio allocation
                    sim.c[i,t] = interp_2d(par.s_grid,par.Rb_grid,sol.c[t,r],sim.s[i,t],sim.Rb[i,t])
                    sim.c[i,t] = np.fmax(sim.c[i,t],0.0) # avoid negative consumption
                    
                    sim.x[i,t] = interp_2d(par.s_grid,par.Rb_grid,sol.x[t,r],sim.s[i,t],sim.Rb[i,t])
                    ...
                    if t<par.simT-1:
                        sim.Rb[i,t+1] = np.fmin(par.Rb_grid[-1],self.Rb_next_func(r,sim.Rb[i,t],sim.eps[i,t+1]) ) # cap at maximum in grid for stability
                        ...
                        # added to answer questions
                        sim.R[i,t+1] = (1-sim.x[i,t])*par.Ra + sim.x[i,t]*sim.Rb[i,t+1] # individual return next period
                        
                        if par.Nr>1:
                            prob_recession = par.prob_recession_0 if sim.r[i,t]==0 else par.prob_recession_1
                            if (sim.draws_r[t] <= prob_recession):
                                sim.r[i,t+1] = 1.0
                            else:
                                sim.r[i,t+1] = 0.0
                        else:
                            sim.r[i,t+1] = par.recession_grid[0] # only one state in the economy
